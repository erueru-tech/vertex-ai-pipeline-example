...
from src.components.operator.add.component import add
...
def pipeline(
    ...
    multiply_task = multiply(v1=v1, v2=v2)
    add(v1=multiply_task.outputs["product"], v2=v2)


def _parse_pipeline_args(args: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument("--v1", default="0.0")
    parser.add_argument("--v2", default="0.0")
...
def main(argv: list[str]) -> None:
    ...
    client = PipelineClient(
        ...
        network=NETWORK,
        parameter_values={"v1": args.v1, "v2": args.v2},
    )

    if args.schedule:
        client.create_schedule(cron="TZ=Asia/Tokyo */5 * * * *", pause=args.pause)
    if args.job:
        client.submit_job()
